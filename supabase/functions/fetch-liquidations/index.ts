...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[CoinGlass] Fetching liquidations for ${symbol}`);
    ...
    console.log(`[CoinGlass] Liquidations fetched successfully`);

    return new Response(JSON.stringify(data), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('[CoinGlass] Error in fetch-liquidations:', error);
    return new Response(
      JSON.stringify({ last24h: { totalLongs: 0, totalShorts: 0 }, timestamp: Date.now() }),
      {
        status: 200,
