...
function calculateSetupScore(
  ...
  const oiExpanding =
    candle.oi && prevCandles.length > 5 ? candle.oi > prevCandles[prevCandles.length - 5].oi! * 1.05 : false;
...
function detectWRFormation(candles: Candle[], type: "buy" | "sell"): boolean {
  ...
  if (type === "buy") {
    ...
    if (
      Math.abs(firstLow.idx - secondLow.idx) > 3 &&
      Math.abs(firstLow.price - secondLow.price) / firstLow.price < 0.01
    ) {
      return true;
    }
  } else {
    ...
    if (
      Math.abs(firstHigh.idx - secondHigh.idx) > 3 &&
      Math.abs(firstHigh.price - secondHigh.price) / firstHigh.price < 0.01
    ) {
      return true;
    }
  }
...
function calculateEntryValidation(
  ...
  const bubbleBurst =
    candle.volume > avgVolume * 2 &&
    ((signalType === "buy" && candle.close > candle.open) || (signalType === "sell" && candle.close < candle.open));
...
async function fetchCoinGlassOHLC(
  ...
  try {
    ...
    const candles: Candle[] = chartResponse.data.map((item: any) => {
      ...
      return {
        ...
        sentiment,
        rsi: clampedRSI,
        oi: parseFloat(item.volume_usd) || undefined,
      };
    });
...
serve(async (req) => {
  ...
  try {
    ...
    if (assetType !== "crypto") {
      throw new Error(`Symbol ${symbol} is not a supported cryptocurrency.`);
    }

    const candles4h = await fetchCoinGlassOHLC(symbol, 240, days, COINGLASS_API_KEY);

    if (!candles4h || candles4h.length === 0) {
      throw new Error(`Failed to fetch CoinGlass data for ${symbol}.`);
    }

    const candles1h = candles4h;
    const candles15m = candles4h;
    const candles5m = candles4h;
    const candles1m = candles4h;
    ...
    return new Response(
      JSON.stringify({
        error: `Chart data limited by provider.`,
        details: errorMessage,
        suggestion: "Backend uses 4h intervals with Hobbyist plan.",
      }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
