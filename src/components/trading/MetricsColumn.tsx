...
export function MetricsColumn({
    ...
    const fetchAllMetrics = async () => {
      ...
      try {
        ...
        const {
          data: frData,
          error: frError
        } = await supabase.functions.invoke("fetch-funding-rate", {
          body: {
            symbol: baseSymbol,
            interval: 'h1'
          }
        ...
        const {
          data: oiData,
          error: oiError
        } = await supabase.functions.invoke("fetch-open-interest", {
          body: {
            symbol: baseSymbol,
            interval: 'h1'
          }
        ...
        const {
          data: lsrData,
          error: lsrError
        } = await supabase.functions.invoke("fetch-long-short-ratio", {
          body: {
            symbol: baseSymbol,
            interval: 'h1'
          }
        ...
        const {
          data: liqData,
          error: liqError
        } = await supabase.functions.invoke("fetch-liquidations", {
          body: {
            symbol: baseSymbol
          }
        });
  ...
  if (loading) {
    return <div className="space-y-2">
        {[...Array(6)].map((_, i) => <Card key={i} className="glass-panel">
  ...
  return <div className="space-y-2">
      ...
      <Card className="glass-panel border border-border/50">
        ...
        <CardContent className="pb-2 space-y-1">
          <div className="flex items-baseline justify-between">
            ...
            {currentFundingRate > 0 ? <div className="flex items-center gap-1 text-chart-green">
                <TrendingUp className="h-3 w-3" />
                <span className="text-[9px] font-semibold">BULLISH</span>
              </div> : <div className="flex items-center gap-1 text-chart-red">
          ...
          <div className="h-1 bg-muted rounded-full overflow-hidden">
            <div className={`h-full transition-all duration-300 ${currentFundingRate > 0 ? 'bg-chart-green' : 'bg-chart-red'}`} style={{
            width: `${Math.min(Math.abs(currentFundingRate * 10000), 100)}%`
          }} />
          </div>
          
          {fundingRate?.current?.nextFunding && <div className="text-[9px] text-muted-foreground font-medium">
      ...
      <Card className="glass-panel border border-border/50">
        ...
        <CardContent className="pb-2 space-y-1">
          ...
          {openInterest?.current?.change24h !== undefined && <div className="flex items-center gap-1">
      ...
      <Card className="glass-panel border border-border/50">
        ...
        <CardContent className="pb-2 space-y-2">
          ...
          <div className="h-1 bg-muted rounded-full overflow-hidden flex">
            <div className="bg-chart-green" style={{
      ...
      <Card className="glass-panel border border-border/50">
        ...
        <CardContent className="pb-2 space-y-2">
          ...
          <div className="h-1 bg-muted rounded-full overflow-hidden flex">
            <div className="bg-chart-red" style={{
            width: `${longLiqPercentage}%`
          }} />
      ...
      {/* Market Dominance */}
      <Card className="glass-panel border border-border/50 bg-[#070c36]">
        <CardHeader className="pb-1 pt-2">
          <CardTitle className="text-[10px] font-semibold tracking-wide text-muted-foreground">
            MARKET DOMINANCE
          </CardTitle>
